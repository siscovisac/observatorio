<template>
    ...
    <div class="card">
        <div class="card-header text-center bg-primary">
            <strong>MANTENIMIENTO - GENÉRICOS</strong>
        </div>
        <div class="card-body">
            ...
            <table class="table table-bordered table-striped table-sm table-responsive-sm">
                ...
                <tbody>
                    <tr v-for="generico in ArrayGenerico" :key="generico.id">
                        <td class="text-center">
                            <button type="button" @click="abrirModal('actualizar',generico)" class="btn btn-primary btn-sm" data-toggle="modal" title="Modificar" data-target="#muestraModal">
                                <i class="fa fa-pencil-alt"></i>
                            </button>
                        </td>
                        <td class="text-center">
                            <template v-if="generico.estado">
                                <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="tooltip" data-placement="bottom" title="Desactivar" @click="desactivar(generico.id)">
                                    <i class="fa fa-check"> A</i>
                                </button>
                            </template>
                            <template v-else>
                                <button type="button" class="btn btn-outline-danger btn-sm" data-toggle="tooltip" data-placement="bottom" title="Activar" @click="activar(generico.id)">
                                    <i class="fa fa-trash-alt"> D</i>
                                </button>
                            </template>
                        </td>
                        <td v-text="generico.nombre"></td>
                        <td v-text="generico.descripcion"></td>
                    </tr>
                </tbody>
            </table>
        </div>
                    ...
                    <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" v-model="nombre" class="form-control focusNext" tabindex="1" maxlength="45" placeholder="generico" autofocus>
    ...
    methods: {
        ...
        desactivar(id) {
            ...
            .then(confirma => {
                if (confirma) {
                    let me=this;
                    axios.put("/generico/desactivar", { id: id }).then(function(response) {
                        me.listarRegistro();
                    });
                } else {
                    swal.close();
        ...
        activar(id) {
            ...
            if (confirma) {
                let me=this;
                axios.put("/generico/activar", { id: id }).then(function(response) {
                me.listarRegistro();
                });
            } else {
                swal.close();
